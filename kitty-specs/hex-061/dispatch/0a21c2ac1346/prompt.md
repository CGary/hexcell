...
verify:
  commands:
    - bash -c 'test -f crates/hexcell-storage/tests/estres_conmutacion.rs'
    - bash -c 'grep -q "fn estres_conmutacion_veinte_lecturas_concurrentes" crates/hexcell-storage/tests/estres_conmutacion.rs'
    - bash -c 'grep -q "estres_conmutacion_veinte_lecturas_concurrentes" .github/workflows/ci.yml'
    - bash -c 'test -f docs/adr/adr-0030-prueba-de-estres-de-conmutacion-de-epoca-bajo-lecturas-concurrentes.md'
    - cargo fmt --check
    - cargo clippy --workspace -- -D warnings
    - cargo test --workspace
    - bash -c 'salida=$(cargo test --workspace -- --ignored estres_conmutacion_veinte_lecturas_concurrentes --nocapture 2>&1); estado=$?; echo "$salida"; test $estado -eq 0 && echo "$salida" | grep -q "1 passed"'
