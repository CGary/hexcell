...
async fn apreton_de_manos_exitoso() {
    ...
    assert_eq!(saludo_nucleo.version, 6);
...
async fn desajuste_de_version_surge_con_ambas_versiones() {
    ...
    assert!(
        mensaje.contains("propia=6") && mensaje.contains("remota=3"),
...
async fn error_de_protocolo_tipo_desconocido() {
    ...
    sidecar
        .enviar_linea_cruda(r#"{"version":6,"tipo":"desconocido"}"#)
        .await;
...
async fn error_de_protocolo_campo_desconocido() {
    ...
    sidecar.enviar_linea_cruda(r#"{"version":6,"tipo":"evento_entrante","id_deduplicacion":"d","id_conversacion":"c","id_remitente":"r","contenido":"x","marca_temporal_ms":0,"campo_extra":1}"#).await;
...
async fn error_de_protocolo_valor_nulo() {
    ...
    sidecar.enviar_linea_cruda(r#"{"version":6,"tipo":"evento_entrante","id_deduplicacion":null,"id_conversacion":"c","id_remitente":"r","contenido":"x","marca_temporal_ms":0}"#).await;
