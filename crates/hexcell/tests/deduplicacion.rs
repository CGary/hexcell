...
//! marcas temporales explícitas suministradas por el propio test: el registro no tiene reloj, así
//! que estos tests tampoco lo necesitan, no duermen nunca y su veredicto es idéntico en cada
//! ejecución.
...
fn un_duplicado_dentro_de_la_ventana_se_descarta_incluso_justo_en_su_borde() {
    ...
    let mut registro = RegistroDeDeduplicacion::nuevo(ventana);
    let id = IdDeduplicacion::nuevo("id-repetido");
    let primera_llegada = SystemTime::UNIX_EPOCH;

    assert_eq!(
        registro.procesar(id.clone(), primera_llegada),
    ...
    assert_eq!(
        registro.procesar(id, justo_antes_del_borde),
...
fn una_reentrega_mas_alla_de_la_ventana_se_procesa_de_nuevo_como_evento_nuevo() {
    ...
    assert_eq!(
        registro.procesar(id, mas_alla_de_la_ventana),
        VeredictoDeDeduplicacion::Nuevo
    );
}
