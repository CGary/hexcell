...
func TestElDocumentoDelProtocoloEstaVersionadoYFechadoEnAbsoluto(t *testing.T) {
	...
	documento := leerDocumento(t)
	if !strings.Contains(documento, "**Versión de este protocolo:** 1.5, fijada el 2026-09-11.") {
...
func TestElDocumentoDeclaraLaCorrespondenciaDeVersiones(t *testing.T) {
	...
	documento := leerDocumento(t)
	if !strings.Contains(documento, "| 1.5 | `6` |") {
		t.Errorf("el documento no declara la correspondencia 1.5 → cable 6")
	}
