...
> Registro vivo del avance. Última actualización: 2026-08-31.
...
## Definido
* **Reversión de época condicionada por re-chequeo estructural/semántico y guardas de fallo silencioso (HEX-057-a, etapa A-5, tarea 8-a).** (2026-08-31, `adr-0026`). Se implementa la secuencia síncrona de reversión en `hexcell_storage::reversion` (`revertir_a_epoca`) y su servicio asíncrono en `hexcell::promocion` (`revertir_epoca_de_conocimiento`): (1) exclusión mutua compartida con promoción mediante `gestor.iniciar_promocion()`, (2) guarda preventiva contra enlace vivo colgante (`verificar_enlace_vivo_resoluble`), (3) resolución de época destino `knowledge_epoch_N.db` reutilizando número y archivo físico sin re-promover ni colisionar (AC-3), (4) prevención de auto-superseído si la época ya es la viva (`EpocaYaEsLaViva`), (5) lectura de sonda semántica persistida (`leer_sonda_semantica`) y auditoría offline (`validar_integridad_del_indice`), (6) partición exhaustiva y disjunta de motivos (`es_motivo_semantico`) entre fallos estructurales (`IntegridadEstructuralRechazada`, GUARD 1) e insuficiencia semántica (`SondaSemanticaRechazada`, GUARD 2) garantizando inercia estricta ante rechazo, (7) resolución canónica ruidosa de la época viva previa antes de mutar enlaces, (8) precalentamiento de conexiones y reasignación atómica de `knowledge_live.db` (`reasignar_enlace_simbolico_vivo`), y (9) swap atómico del pool en memoria (`ArcSwap`) con instrumentación de latencia NFR-03 y entrega de `EpocaSuperseida` para su drenaje ordenado. Adicionalmente, se introduce la guarda contra enlaces colgantes en `GestorDePools::abrir` (GUARD 3, previniendo la creación de bases vacías corruptas) y canonicalización ruidosa en `promover_epoca` (GUARD 4, con aborto limpio y reintentable).
