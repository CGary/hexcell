...
* **Confirmación de eventos entrantes antes del registro durable** (2026-08-08, HEX-015; ratificado por decisión humana). `AdaptadorWhatsmeow` confirma un `evento_entrante` al sidecar tras entregarlo a un `mpsc` en memoria, no tras un registro durable del lado del núcleo, contra lo que exige la sección 4 del protocolo. Un caído del proceso entre ambos puntos degrada la entrega de «al menos una vez» a «como mucho una vez». Cierra cuando el núcleo tenga consumo durable propio del evento; registrado en `adr-0011`. — *Etapa A-3.*
* **Política anti-ban no desactivable por configuración** (2026-08-08, HEX-015). `adr-0011` declara esta política como parte de su alcance, pero su implementación —la disciplina de comportamiento del canal: latencia mínima, indicador de escritura, límite de un mensaje por turno, cortacircuitos conversacional, rampa de volumen— es la tarea 14 de la etapa A-3 y todavía no existe. Sigue abierta hasta que esa tarea se ejecute. — *Etapa A-3.*
