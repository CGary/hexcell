...
> **Registro de decisiones, no de avance.** Última actualización: 2026-09-13.
>
...
## Definido
* **El puerto de notificación vive en `hexcell-core` y sus dos implementaciones —sumidero falso y sumidero Telegram— en `crates/hexcell`, sin crate propio** (2026-09-13, HEX-077-a, sub-tarea 20-a de A-6). Sigue el precedente de `ProveedorDeInferencia`/`ProveedorDeEmbeddings`, no el de `ChannelAdapter`: este último ganó crates propios porque FR-12 exige dos adaptadores coexistiendo en producción con pruebas de contrato cruzadas; las notificaciones tienen un sumidero real y un doble de prueba. Sin dependencia nueva (reutiliza el cliente HTTP ya declarado para el proveedor LLM). El rasgo no es `dyn`-compatible por la compuerta de clippy del proyecto. El token de Telegram entra solo por el puerto de entorno (`HEXCELL_TELEGRAM_BOT_TOKEN`, `HEXCELL_TELEGRAM_CHAT_ID`, `HEXCELL_TELEGRAM_URL_BASE`), nunca por archivo por célula. Sin severidad, umbral, reintento ni deduplicación en el puerto (D-09). El cableado en `main.rs` y el paso de variables en la composición quedan para HEX-077-b.
...
* **La latencia hasta el acuse se entrega como cuarta clave del productor de métricas del sidecar: `latencia_hasta_acuse_ms` en la misma línea periódica `key=value` que `adr-0033` ya define** (2026-09-13, HEX-077-c, `adr-0035`, sub-tarea per-cell-metrics de la tarea 20 de A-6). Se calcula dentro de `ObservarAcuse` sobre el estado ya local al productor —`correlacionPendiente.creadaMs` estampado por `ObservarEnvio` con el mismo reloj inyectado `p.ahoraMs()` que `Instantanea` ya usa para `silencio_entrante_ms`—, clampada a 0 si fuera negativo por la misma defensa que la línea de silencio aplica, y almacenada como un único entero `int64` (`ultimaLatenciaAcuseMs`) que sobre-escribe el previo en cada acuse resuelto: semántica de "última observada", no promedio ni percentil ni segmento por contacto. La emisión conserva el formato `latencia_hasta_acuse_ms=%d` (entero en milisegundos, nunca `%.2f`, nunca JSON, nunca una segunda línea) y se coloca en `Instantanea()` entre `silencio_entrante_ms` y `contactos_omitidos` para que las cinco claves agregadas —las cuatro de `adr-0033` más esta— vayan siempre juntas. No añade tipo IPC, no sube la versión de cable (sigue en 6, `adr-0032`), no toca `sidecar/internal/canal/acuses.go` ni `sidecar/main.go` (la firma de `ObservarAcuse(idCorrelacion, estado)` y su único call site ya reciben todo lo necesario), y mantiene `sidecar/go.mod` y `sidecar/go.sum` byte-idénticos. Huella: 8 bytes (un `int64`). Cubre por mutación tres de los cuatro frentes de fallo (la cuarta documenta el valor por omisión y no puede ponerse roja con esa mutación) con `TestLatenciaHastaAcuseCalculadaSobreAcuseConocido`, `TestLatenciaHastaAcuseValeCeroAntesDeCualquierAcuse`, `TestLatenciaHastaAcuseReflejaElMasRecienteNoElPrimero` y `TestLatenciaHastaAcuseNoSeMuevePorAcuseHuerfano`, todos en verde bajo `go test ./internal/metricas/ -count=1`. La tarea 20 del plan queda cerrada en su sub-tarea per-cell-metrics: HEX-077-b puede consumir la nueva clave cuando implemente las ocho condiciones de alerta, HEX-077-a el puerto de notificación, y HEX-077-d el dead-man's switch —cada uno por su lado, sin acoplamiento—. Verificación de las otras dos series per-cell ya entregadas por `adr-0033`: `TestReconexionesPorHoraCuentaTransicionesNoEstadosRepetidos` y `TestSilencioEntranteCreceYSeReiniciaConObservarEntrante` siguen verdes sin modificación. Promedio móvil descartado en D-50; segmento por contacto descartado por contrato (la guarda `adr-0019` aplicaría y la cardinalidad de la métrica agregada quedaría rota, dos costes que la promesa del plan no pide).
...
* **Registro de imágenes de contenedor para la CI de la tarea 18.** (2026-09-10). La tarea 18 construye y publica las dos imágenes de la célula desde la CI; qué registro recibe esas imágenes es decisión aún no tomada. — *Etapa A-6, tarea 18.* *(Sustituida el 2026-09-23 por la entrada siguiente, escrita al cerrar HEX-086; se conserva por trazabilidad.)*
* **Destino del registro de imágenes de la célula: GHCR, registro propio u otro.** (2026-09-22, HEX-086). La tarea 18 cerró con la construcción y el etiquetado de ambas imágenes en la CI —SHA corto de 12 caracteres y versión de `[workspace.package]`— y con la guarda `deploy/verificar_imagenes.sh` verificando el usuario no root y el arranque en frío endurecido, pero **sin publicar nada**: el trabajo `imagenes` de la CI construye con `push:false` y `load:true`, y no existe paso de login ni credencial. Qué registro recibe esas imágenes (GHCR, registro propio del servidor u otro) sigue sin decidirse y **bloquea convertir la tarea 18 en un paso real de publicación**: según la nota de cierre de esa tarea, publicar es un cambio de una línea (`push:true` más un paso de login) una vez decidido el destino. — *Etapa A-6, tarea 18.*
...
* **Recuperación de una reanudación de `cell rebind` que encuentra la sesión ya emparejada** (2026-09-22, HEX-085). La reanudación desde `Reemparejando` salta al paso de emparejamiento; si la célula quedó allí por un `reanudar` fallido o por un fallo entre la persistencia de `Reemparejando` y el arranque del sidecar (pasos 5 a 7), el sidecar puede seguir emparejado y responder `ya_emparejada`, y el comando termina en `Fallo` sin ruta de salida. Qué debe hacer el operador o el comando en ese caso (tratar `ya_emparejada` como confirmación, repetir el descarte del `sqlstore`, u otra vía) no está decidido. — *Etapa A-6, tarea 15.*
